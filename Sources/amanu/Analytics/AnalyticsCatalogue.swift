...
enum AnalyticsCatalogue {
    ...
    static func sanitized(_ properties: [String: Any]) -> [String: Any] {
        ...
        let engines = ["auto", "parakeet", "openai", "assemblyai", "whisperai"]
        ...
        let allowed = [
            ...
            "transcription_cloud_provider": ["assemblyai", "openai", "whisperai"],
    ...
    static func transcriptionModel(engine: String, provenance: String) -> String {
        ...
        case "whisperai":
            // Same shape as assemblyai's provenance: the model, then the
            // language expectation. Only the model half may travel.
            let model = provenance.components(separatedBy: " · ").first ?? provenance
            return model == "default" ? "default" : "custom"
        default:
            return "unknown"
        }
    }
